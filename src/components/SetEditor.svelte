...
<Dialog bind:this={dialog}>
  ...
  <div class="flex flex-col py-2 w-96">
    ...
    <section
      tabindex="-1"
      class="w-full p-px mt-3 space-y-3 overflow-visible overflow-y-scroll transition rounded-lg h-96"
      use:dndzone={{ items, flipDurationMs: 150, dropTargetStyle: '', dropTargetClasses: ['drop-target'], type: 'instrumentList' }}
  ...
  <div class="flex items-center justify-between" slot="bottom-actions">
    {#if $instrumentSets.length > 1}
      <Button label="Delete set" on:click={deleteSet} />
    {/if}

    <ContextMenu
      bind:this={soundfontContextMenu}
      position={displayPositions.topLeft}>
