...
export default class Instrument {
  ...
  getAdjustedNote(noteCode, shiftUp = false, shiftDown = false) {
    return parseInt(noteCode, 10) + 12 * (this.octave + (shiftUp ? 1 : 0) - (shiftDown ? 1 : 0));
  }

  async playNotes(noteCodes, shiftUp = false, shiftDown = false) {
    noteCodes.map((noteCode) => this.playNote(noteCode, shiftUp, shiftDown));
  }

  async playNote(noteCode, shiftUp = false, shiftDown = false) {
    if (this.instrument === undefined) {
      this.instrument = await this.data;
    }

    const note = this.getAdjustedNote(noteCode, shiftUp, shiftDown);
